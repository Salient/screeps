...
function findSource(creep) {
  ...
  // Presort and select a sensible shaft to assign
  var workforce = {};
  var freeShafts = [];
  ...
  for (var post in shafts) {
    ...
    if (!Game.creeps[mineHole.assignedTo] || (mineHole.assignedTo == 'unassigned')) {
      mineHole.assignedTo = 'unassigned';
      freeShafts.push(mineHole);
    } else {
      var manpower = Game.creeps[mineHole.assignedTo].getActiveBodyparts(WORK);

      util.def(workforce[mineHole.srcId])? workforce[mineHole.srcId] += manpower : workforce[mineHole.srcId] = manpower);
    } 
  }

  if (freeShafts.length == 0) {
    return false;
  } else if (freeShafts.length == 1) {
    return freeShafts[0];
  }

  // Yes I know it's stupid to do it this way, but I suspect there may be more than two sources some day.

  var tgtSrcId  = Object.keys(workforce)[0];
  var needStaff = workforce[tgtSrcId];

  for (var team in workforce) {
    if (workforce[team] < needStaff) {
      tgtSrcId = team;
      needStaff = workforce[team];
    }
  }

  for (var ass in freeShafts) {
    if (freeShafts[ass].srcId == tgtSrcId) {

      if (creep.memory.role == 'miner') {
	dlog('assinging shaft at ' + freeShafts[ass].pos.x + ',' + freeShafts[ass].pos.y + ' to ' + creep.name);
	freeShafts[ass].assignedTo = creep.name;
      } else {
	dlog('lending shaft at ' + freeShafts[ass].pos.x + ',' + freeShafts[ass].pos.y + ' to ' + creep.name);
      }
      return freeShafts[ass];
    }
  }
  dlog('this is a bad place');
  return false;
}
