...
// if (!nrg) {
// dlog('nothing stored?')
// dlog('builders need energy to build, but none stored available. reverting
// tasks');
// creep.memory.taskList.pop();
// return false;
// }
...
// case ERR_NOT_IN_RANGE:
// var path = creep.moveTo(gas, {reusePath: 5, visualizePathStyle: {stroke:
// '1ffaa00'}});
// if (path != OK && path != ERR_TIRED) {
...
// }
// return;
// if (!util.def(creep.memory.wTarget) ||
// !util.def(Game.getObjectById(creep.memory.wTarget)) ||
// Game.getObjectById(creep.memory.wTarget).energy < 50){
//
...
// if ((((priority == 'container') || (priority == 'storage')) &&
// (structs[site].store >=50)) ||
// (((priority == 'extension') || (priority == 'spawn')) &&
// (structs[site].energy >=50))) {
// // dlog('passed sniff test')
// var res = creep.moveTo(structs[site], {reusePath: 5, visualizePathStyle:
// {stroke: '1ffaa77'}});
// if (!res){
...
// var path = creep.moveTo(sugarDaddy.pos, {reusePath: 5, visualizePathStyle:
// {stroke: '1ffaa00'}});
// if (path != OK && path != ERR_TIRED) {
