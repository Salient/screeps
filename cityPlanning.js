...
Room.prototype.placeLinks = function() {
	...
	this.placeAuxLink = function(srcId) {
		...
		for ( var ind in shafts) {
			...
			var x = 1, rx = thisShaft.pos.x, // W component of current room
			ry = thisShaft.pos.y, // N component of current room
			vector = util.spiral(x);

			// return;
			while (vector[2] <= 2) {
				var nx = +rx + +vector[0], ny = +ry + +vector[1];
...
if (util.def(this.memory.primeLinkId)) {
	...
	if (util.def(this.memory.primeLinkLoc)) {
		...
		var bud = this.lookForAt(LOOK_CONSTRUCTION_SITES,
				this.memory.primeLinkLoc.x, this.memory.primeLinkLoc.y);
		for ( var hmm in bud) {
			var derp = bud[hmm];
		...
		if (primeLink != 'building') {
			...
			var bud = this.lookForAt(LOOK_STRUCTURES,
					this.memory.primeLinkLoc.x, this.memory.primeLinkLoc.y);
...
// Want to place it within reach of a miner, so behind a mineshaft, but leave
// room for pathing
}
